...
test.serial.skip("pkg.bin as a string should work", t => {
...
test("single character flag casing should be preserved", t => {
	t.deepEqual(cliLab({ argv: ["-F"] }).flags, { F: true });
});

test("type inference", t => {
	t.is(cliLab({ argv: ["5"] }).input[0], "5");
	t.is(cliLab({ argv: ["5"] }, { input: "string" }).input[0], "5");
	t.is(
		cliLab({
			argv: ["5"],
			inferType: true
